...
export function decryptBase64RSA(publicKeyString: string, signature: string, verifyCode: { userId: string }) {
    let isVerified = false;
    const key = new NodeRSA();
    const signer = key.importKey(Buffer.from(publicKeyString, 'base64'), 'pkcs8-public-der');
    if (verifyCode.userId) {
        isVerified = signer.verify(Buffer.from(verifyCode.userId), signature, 'utf8','base64');
    }
    return isVerified;
}	